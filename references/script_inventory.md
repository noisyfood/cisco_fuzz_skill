...
| `scripts/local_cli_mutation_fuzzer.py` | Yes, smoke only | Primitive local smoke supervisor for file-input CLI targets. It validates mutation plumbing and capture, but is not a final campaign fuzzer. Supports raw token files and AFL dictionary escape syntax. | Local target command is safe to execute; use a wrapper that maps expected parser rejects to exit 0 and preserves signal-like exits, sanitizer output, and timeouts. |
...
| `scripts/live_probe_executor.py` | Yes, with strict gate | Low-rate live baseline/probe/replay executor for TCP/UDP request/response targets; not a campaign fuzzer. | Valid `cisco_live` manifest; `--lab-mode` is only for loopback smoke tests unless private lab mode is explicit. |
...
The Rust LibAFL template under `assets/rust_libafl_cli_command_fuzzer/` is directly usable after `cargo check`. Use it for local file-input CLI targets where a Rust LibAFL executor is warranted; it supports `--token-file` for raw or AFL dictionary tokens.

The Rust LibAFL forkserver template under `assets/rust_libafl_afl_forkserver_fuzzer/` is directly usable after `cargo check`. Use it for targets compiled with AFL-compatible instrumentation, including partial-instrumentation source harnesses. AFL++ provides the compiler/runtime handshake; LibAFL remains the fuzzer.
