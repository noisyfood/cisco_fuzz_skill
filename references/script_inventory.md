...
| `scripts/live_driver_gate.py` | Yes | Pre-traffic gate for protocol-specific Cisco live drivers and replay helpers; checks destructive actions against the manifest allowlist. | Valid `cisco_live` manifest and matching runtime host/proto/port/seed/case arguments. |
| `scripts/live_probe_executor.py` | Yes, with strict gate | Profile-gated live baseline/seed-replay executor for TCP/UDP request/response targets; not a campaign fuzzer and does not mutate inputs. | Valid `cisco_live` manifest; `--lab-mode` is only for loopback smoke tests unless private lab mode is explicit. `destructive_lab` uses `destructive_lab.campaign_budget.max_cases` instead of `live_safety.max_first_contact_cases`. |
